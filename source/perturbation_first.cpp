...
double first_order(double mass, double z_ini, double rtols[3], double atols[3], double r_here, int N_GaussLaguerre, double Tnu) {
    ...
    auto [I, err] = GaussKronrod(integrand_z, 0.0, z_ini, rtols[0], atols[0], args);
    return I;
};
...
double integrand_y(double y, FirstOrderArguments args) {
    ...
    auto [I, err] = GaussKronrod(integrand_theta, 0.0, std::numbers::pi, args.rtols[1], args.atols[1], args);
    return I;
};
